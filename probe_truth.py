...
for data in statements:
    ...
    for l in layer_activations:
        layer_activations[l].append(cache[f"blocks.{l}.hook_resid_post"].squeeze(0)[-1].cpu()) #last token position [-1], now mean across activations mean(dim=0)
    del cache, logits
...
ax1.set_ylim(0, 1)
fig1.tight_layout()
fig1.savefig("/workspace/truth_probe_accuracy_by_layer_cities_last_regularized.png", dpi=150)
...
fig2.tight_layout()
fig2.savefig("/workspace/truth_probe_contributions_cities_last_regularized.png", dpi=150)
