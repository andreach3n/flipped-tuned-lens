...
print(f"[check_saes] ARM={ARM} model={model_path} sae={SAE_DIR} layer={LAYER} device={device}")
sae = SparseCoder.load_from_disk(os.path.join(SAE_DIR, f"layers.{LAYER}"), device=str(device))
...
h_sum = t.zeros(model.config.hidden_size, dtype=t.float64, device=device)
h_sqsum = t.zeros(model.config.hidden_size, dtype=t.float64, device=device)

with t.no_grad():
    for i in range(0, n_seq, BATCH):
        batch = toks[i : i + BATCH].to(device)
        model(batch)
        h = captured["h"].reshape(-1, model.config.hidden_size).float()
        out = sae(h)
...
mu = h_sum / n_tok
var_cent = float((h_sqsum / n_tok - mu ** 2).sum())
sst_cent = var_cent * n_tok
sst_flat = float((h_sqsum).sum())
