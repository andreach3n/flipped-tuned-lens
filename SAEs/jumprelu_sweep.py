...
results_arr = np.array(results)                       # [n_runs, 4]: coeff, L0, fvu, dead
...
# --- plot (single seed: one row per coefficient) ---
coeffs = results_arr[:, 0]
L0s    = results_arr[:, 1]
fvus   = results_arr[:, 2]
deads  = results_arr[:, 3]
...
# Panel 1: the plateau plot. A flat band = a "sticky" L0.
